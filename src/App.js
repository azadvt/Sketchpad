...
function App() {
  ...
  useEffect(()=>{
    if(!brush) {
      canvasStore.current.freeDrawingBrush = new fabric.EraserBrush(canvasStore.current) ;
      canvasStore.current.freeDrawingBrush.width = 5; //minimum width
      canvasStore.current.isDrawingMode = true;
    }
    else{
      canvasStore.current.freeDrawingBrush = new fabric.PencilBrush(canvasStore.current)
    }
  },[brush])
